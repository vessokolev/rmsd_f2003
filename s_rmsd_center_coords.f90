subroutine s_rmsd_center_coords(num_atoms,do_count,x,ncount)
!
! This subroutine is part of the implementation of Quaternion Characteristic Polynomial (QCP) method,
! based on the code published by Naoto Hori at:
!
...
! The code bellow is improved and partially revised by Veselin Kolev <vesso.kolev@gmail.com>.
!
! To cite QCP:
!
...
real(C_FLOAT),intent(inout) :: x(3,num_atoms)
...
! Nullify the components of the vector 's':
!
s=0.0
!
! Initialize ncount
...
! components of the vector 's'. Count only the atoms for which
